...
1. 确认已安装 Obsidian 桌面版（最低版本 1.8.0）
2. 确认项目已构建：运行 `npm run build` 生成 `main.js`
3. 将以下三个文件复制到你的 Vault 的 `.obsidian/plugins/agent-dashboard/` 目录：
   - `main.js`
   - `manifest.json`
   - `styles.css`
4. 打开 Obsidian，进入 设置 → 第三方插件，关闭"安全模式"
5. 在插件列表中找到"墨忆台 Memory Workbench"，点击启用
