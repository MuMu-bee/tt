...
import type { ChangeKind, AuditEvent, RefreshStatus, WriteRequest, WriteResult } from '../application/contracts.ts';
...
export class WriteService {
  ...
  constructor(port: WritePort, audit: AuditSink, lifecycle?: IndexLifecycleService, flags: FeatureFlags = DEFAULT_FEATURE_FLAGS) {
    this.port = port;
    this.audit = audit;
  ...
  async write(request: WriteRequest, context: RequestContext): Promise<WriteResult> {
    ...
    try {
      ...
      return result;
    } catch (error) { const result = this.result(request, 'failed', 'WRITE_FAILED', beforeHash); await this.appendAudit(request, result, context); return result; }
  }
